...
class HEIsingleDataset(BaseDataset):
    """
    This dataset class loads synthetic unaligned/unpaired datasets for shadow removal tasks.
    ...
    def __getitem__(self, index):
        """Return a test pair and associated data.

        Args:
            index: Index of the dataset item.

        Returns:
            dict: Contains tensors for shadow image, HMC processed data, and paths.
        """
        # Get paths for shadow image and mask
        ...
        DE, umbra, edge_mask, umbra_mask = self.apply_hei_processing(
            A_img_np, A_mask_np, self.opt
        )
    ...
    def apply_hei_processing(self, shadow_img, mask, opt):
        """Apply HEI shadow removal processing.
